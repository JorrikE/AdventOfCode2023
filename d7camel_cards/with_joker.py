...
def get_hand_code(hand_str: str) -> list[int]:
    if "J" in hand_str:
        hand_str: str = _process_wildcard(hand_str)
    return _get_hand_code(hand_str)


def _get_hand_code(hand_str: str) -> list[int]:
    hand_code: list[int] = []
...
def _process_wildcard(hand_str: str) -> list[int]:
    if hand_str == "JJJJJ":
        return hand_str
    non_joker_cards: str = set(hand_str.replace("J", ""))
    most_frequent_card: str = None
    most_frequent_occurence: int = 0
    for card_str in non_joker_cards:
        card_occurence: int = hand_str.count(card_str)
        if card_occurence > most_frequent_occurence:
            most_frequent_card = card_str
            most_frequent_occurence = card_occurence
    return hand_str.replace("J", most_frequent_card)
